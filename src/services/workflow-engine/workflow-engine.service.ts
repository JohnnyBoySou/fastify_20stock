import { FlowNode, ExecutionContext, ConditionConfig, ActionConfig } from '@/features/flow/flow.interfaces';
...
export const WorkflowEngine = {
  ...
  async executeNodes(nodes: FlowNode[], edges: any[], context: ExecutionContext, state: ExecutionState) {
  ...
  async executeFlowFromNode(
    ...
    try {
      switch (node.type) {
        ...
        case 'condition':
          const conditionResult = await this.evaluateCondition(node.data.config as ConditionConfig, context);
        ...
        case 'action':
        case 'notification':
          const actionResult = await ActionExecutor.executeAction(node.data.config as ActionConfig, context);
  ...
  async evaluateCondition(conditionConfig: ConditionConfig, context: ExecutionContext): Promise<boolean> {
