import { FlowNode, FlowEdge } from '@/features/flow/flow.interfaces';
...
export const WorkflowValidation = {
  ...
  detectInfiniteLoops(nodes: FlowNode[], edges: FlowEdge[], maxDepth: number = 100): string[] {
    const errors: string[] = [];
    ...
    const hasCycle = (nodeId: string, depth: number): boolean => {
      if (depth > maxDepth) {
        errors.push(`Potential infinite loop detected starting from node "${nodeId}" (depth > ${maxDepth})`);
        return true;
      }
