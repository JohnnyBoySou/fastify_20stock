...
export const FlowController = {
  ...
  async getByStore(request: FastifyRequest<{ Querystring: { storeId?: string } }>, reply: FastifyReply) {
    try {
      const storeId = request.store?.id;
  ...
  async getStats(request: FastifyRequest<{ Querystring: { storeId?: string } }>, reply: FastifyReply) {
    try {
      const storeId = request.store?.id;
